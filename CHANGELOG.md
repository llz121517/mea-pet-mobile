...
- **发送失败留下"幽灵消息"，污染后续上下文并使重试后消息重复** — `ChatService.sendMessage` 在请求**之前**就把用户消息写进 `ConversationManager`（`apiMessages` 由历史构建，必须如此），但两条失败路径（响应无有效内容、异常）都直接返回 `Result.failure`，**从不回滚**。后果有两个：一是这条从未成功发出的消息被持久化并进入后续每一次请求的 `messages`，出现两条连续 user 而中间无 assistant，模型可能改去回答上一条失败的消息；二是它与 `ChatViewModel` 的乐观消息内容相同但 **id 不同**，此后任意一次 `ON_RESUME`（息屏亮屏 / 切后台 / 悬浮窗返回）触发的 `reloadHistory()` 会把两者并列成两条相同的用户气泡——用户表现为「重试后消息发了两遍，但只记录了一次」，且因需要两个条件凑齐而呈偶发。现三处同修：失败路径（含 `CancellationException`）统一 `removeMessage(userMessage.id)` 回滚；新增 `sendMessage(userMessage: ChatMessage)` 重载让 UI 乐观消息与入史消息共用同一 id（`retryLastMessage` 也改为复用原消息而非按内容新建，重试前后 id 与 timestamp 保持稳定）；`mergeWithHistory` 的 `idxInCurrent == -1` 兜底分支追加时间戳过滤，只保留确实晚于历史末条的消息，符合其「history 之后才是新追加」的合并前提。补 3 条回归用例（失败后历史应为空、后续请求体不含失败消息、resume 后不重复）。
- **`retryLastMessage` 在 `cutIndex == -1` 时会清空界面全部 AI 回复** — `ChatViewModel` 的 `filterIndexed` 以 `cutIndex` 为分界，目标消息不在列表时 `cutIndex = -1` 使所有下标都落进 else 分支（`!msg.isAssistant`），等于把界面上所有 assistant 消息删掉。补 `cutIndex < 0` 分支：原样保留列表、只追加新消息。
- **宽表格被挤到逐字换行** — 助手/用户气泡宽度原写死 `widthIn(max = 280.dp)`，与屏幕尺寸无关（平板同样只有 280dp）；而 Markwon 的 `TableRowSpan` 把可用宽度按列均分（`cellWidth = width / columns`）、`getSize()` 只回报上次记录的宽度、从不为表格索取更多空间，也没有横向滚动。5 列时每列仅约 49dp，`列1`~`列5` 表头被折成「列 / 数字」两行、连读成 `列列列列列`。现三处同修：①气泡宽度改为 `min(屏幕宽 × 0.82, 460dp)`；②`MarkdownText` 用 `splitMarkdownTables` 按 GFM 表格块把正文切段（围栏代码块内的 `|---|` 不误判），普通段落各按气泡宽度正常换行，只有所需宽度（每列不少于 88dp）超出气泡的表格才套横向滚动容器——表格与正文若共用一个 TextView，正文会被迫按表格宽度排版、横向溢出气泡；③滚动容器用 `FixedContentWidthScrollView` 而非原生 `HorizontalScrollView`：后者重写了 `measureChild` / `measureChildWithMargins`，一律以 `UNSPECIFIED` 规格量子 View 宽度、**完全忽略 `layoutParams.width`**（横向滚动容器的语义本就是「子 View 想多宽给多宽」），于是 TextView 只能按内容算宽度，而 `TableRowSpan.getSize()` 在首次 draw 前返回 0（宽度只在 `draw()` 里由 `SpanUtils.width` 赋值），表格独占的 TextView 会量出宽 0、根本画不出来；子类改用 `EXACTLY` 规格下发内容宽度，宽度才真正生效。气泡左右内边距随之从 `Box` 移进 `MarkdownText`：对正文是外部 padding，对表格则加在滚动容器上并配 `clipToPadding=false`，静止时首末列留出内边距，滚动时内容可画进内边距区域，不会在离气泡边缘还有一段距离的位置就凭空消失（`SpanUtils.width` 取的是 `textView.width` 减左右 padding，内边距因此只能放容器、不能放 TextView，否则列宽会把内边距算进去）。无表格的消息完全走原路径不受影响。新增 `markdownTableColumns` 按 GFM 分隔行数列数、`splitMarkdownTables` 负责切段，配 11 条用例（分割线 `---` 不误判、省略首尾竖线、多表格取最大值、表格前后文字各自成段、围栏代码块内 `|---|` 不误判、表格紧接文字无空行也能切开）。
- **重试会重发上一条消息并导致应用崩溃** — 上面那条「失败回滚」把 `retryLastMessage` 的隐含前提抽掉了：它原本从 `conversationManager.lastUserMessage()` 取重试目标，而回滚后失败的消息已不在历史里，取到的是**上一条早已成功的消息**。于是重试重发了那条旧消息，而它的原副本还在 UI 列表里 → 同一个 id 出现两份 → `LazyColumn` 抛 `IllegalArgumentException: Key "…" was already used` **直接崩溃**（真机实测连崩两次，两次同一个 id）。改为 `retryMessage(userMessage: ChatMessage)`：重试目标由 UI 传入（UI 才知道用户点的是哪条），历史里找不到时按新消息重发而不再回退到「最后一条 user」；`drop(userIdx + 1)` 只在 `userIdx >= 0` 时执行，因此上一批为 `userIdx < 0` 加的守卫由结构本身保证。`ChatViewModel` 侧的过滤同时改为「按 id 一律剔除目标消息」，不再只看 `index <= cutIndex`，避免任何位置的重复副本被漏掉。补 3 条回归用例。
- **消息列表重复 id 会崩掉整个应用** — `LazyColumn` 撞到重复 `key` 抛的是异常而非降级渲染，任何一处产生重复 id 的上游 bug都会表现为闪退。在唯一消费点 `MessageList` 加一道 `distinctBy { it.id }` 兜底（真正的修复仍在产生 state 处），把这类问题的后果从崩溃降为少显示一条。
...
- 第三批是两条用户实测反馈的 bug（宽表格排版错乱、重试后消息重复），**两者都不在那份 48 条审查报告里**——重复消息一条串起了四处代码（失败不回滚 → 双 id → resume 合并兜底 → 重试按 id 剔除），先用单元测试确定性复现后再修。**AI 审查漏掉的恰恰是用户唯一能看见的那类问题。**
